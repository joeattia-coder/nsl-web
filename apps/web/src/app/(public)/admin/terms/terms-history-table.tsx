...
import type { TermsVersionRow } from "./terms-form";

type TermsHistoryTableProps = {
...
export default function TermsHistoryTable({ versions }: TermsHistoryTableProps) {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.details || data?.error || "Failed to delete Terms history entry.");
      }
      ...
      setError(
        deleteError instanceof Error
          ? deleteError.message
          : "Failed to delete Terms history entry."
      );
    } finally {
      setDeletingId(null);
    }
                      ...
                      <td>
                        {version.isLatest ? (
                          ...
                          <button
                            ...
                            aria-label={`Delete archived terms version from ${version.publishedAt}`}
