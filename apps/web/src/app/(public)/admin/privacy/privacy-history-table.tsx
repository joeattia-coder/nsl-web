...
import type { PrivacyVersionRow } from "./privacy-form";

type PrivacyHistoryTableProps = {
  versions: PrivacyVersionRow[];
};
...
export default function PrivacyHistoryTable({ versions }: PrivacyHistoryTableProps) {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.details || data?.error || "Failed to delete Privacy Policy history entry.");
      }
      ...
      setError(
        deleteError instanceof Error
          ? deleteError.message
          : "Failed to delete Privacy Policy history entry."
      );
    } finally {
      setDeletingId(null);
    }
                ...
                {sortedVersions.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="admin-players-empty">
                      No Privacy Policy history entries yet.
                    </td>
                  </tr>
                ) : (
                      ...
                      <td>
                        <div className="flex min-w-[220px] flex-col gap-1">
                          ...
                          <span className="text-xs text-slate-500 line-clamp-2">
                            {summarizeHtml(version.contentHtml) || "No content"}
                          </span>
                        </div>
                      </td>
                      ...
                      <td>
                        {version.isLatest ? (
                          ...
                          <button
                            ...
                            aria-label={`Delete archived privacy policy version from ${version.publishedAt}`}
