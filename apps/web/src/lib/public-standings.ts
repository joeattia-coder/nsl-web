...
export type PublicStandingsGroup = {
...
export async function getPublicTournamentStandings(
  fixtureGroupIdentifier: string
): Promise<PublicStandingsResponse> {
  ...
  const resultGroups = groups.map((group) => {
    ...
    const participantEntryIds = new Set(group.participants.map((participant) => participant.tournamentEntryId));
    ...
    if (matchesForStandings.length === 0) {
      matchesForStandings = group.stageRound.matches.filter((match) => {
        return participantEntryIds.has(match.homeEntryId) && participantEntryIds.has(match.awayEntryId);
      });
    }
    ...
    return {
      ...
      count: rows.length,
      rows,
    } satisfies PublicStandingsGroup;
  });
  ...
  return {
    ...
    groups: resultGroups,
  } satisfies PublicStandingsResponse;
}